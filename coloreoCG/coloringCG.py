...
if __name__ == "__main__":
    ...
    #n_nodos, n_aristas, adj = parserDimacs.parserDimacs("coloreoCG/DSJC250.5.col")
    #n_nodos, n_aristas, adj = parserDimacs.parserDimacs("coloreoCG/DSJC250.9.col")
    #n_nodos, n_aristas, adj = parserDimacs.parserDimacs("flat300_20_0.col")
    ...
    for v in range(1,n_nodos+1):
        var = model.addVar(name=f"{v}_", vtype="C", lb=0.0, ub=None, obj=2)
    ...
    while(i<=max_it):
        print(f"Iteración {i}")
        prev_nodes_weights = dict(nodes_weights)
        model.optimize()
        ...
        for v in range(n_nodos):
            ...
            #nodes_weights[v+1] = pi
            
        print(f"NODES WEIGHTS BEFORE: {prev_nodes_weights}")
        print(f"NODES WEIGHTS AFTER: {nodes_weights}")
        ...
        S,w = heuristics.greedy2(nodes_weights,adj)
        ...
        S2,w2 = heuristics.greedy1(nodes_weights,adj)
        if w2 > bestW : 
            bestW = w2
            bestS = S2
            bestStrat = "Greedy1"
        ...
        if bestW > 1.0:
            ...
            model.freeTransform()
            new_col_var = model.addVar( name=varName,vtype="C", obj=1.0, lb=0.0, ub=None)
            for v in bestS:
                print(v)
                print(f"Constraint: {v} - {model.getConss()[v-1]}")
            ...
            S = {}
            F = dict(prev_nodes_weights)
            X = set()
            mwssSol,mwssW = mwssRecursion.mwssRecursion(S=S,F=F,X=X,adj=adj,maxIt=10)
            ...
            if mwssSol:
                ...
                #print(f"New MWSS VAR: {new_mwss}")
                new_mwss = model.addVar( vtype="C", obj=1.0, lb=0.0, ub=1.0)
                for v in mwssSol:
                    print(f"V in mwssSol: {v}")
                    #model.addConsCoeff(constraints[v], new_mwss, 1.0)
                    print(f"Cons{v} in model: {model.getConss()[v-1]}")
                    model.addConsCoeff(model.getConss()[v-1], new_mwss, 1.0)
            print(f"Mwss Solución: {mwssSol}")
            print(f"Mwss Peso: {mwssW}")
            '''
            break   
        i=i+1
    ...
    print(f"MEJOR COLOREO: {model.getObjVal()}")
    ...
    #FILE_PATH = "DSJC125.1.col.b"  # Mejor K conseguido: 10 - Mejor conocido = ?
    #FILE_PATH = "DSJC250.5.col.b"  # Mejor k conseguido = 43 - Mejor conocido = 26
    #FILE_PATH = "DSJC250.9.col.b"  # Mejor k conseguido = 97 - Mejor conocido = 71
    #FILE_PATH = "flat300_20_0.col"  # Mejor k conseguido = 48 - Mejor conocido = 20